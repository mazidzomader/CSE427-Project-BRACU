...
# ── Configuration ────────────────────────────────────────────────────────────
files = {
    "MLP": "D4_mlp_metrics.txt",
    "Random Forest": "D4_rf_metrics.txt",
    "XGBoost": "D4_xgb_metrics.txt",
    "AdaBoost": "D4_ada_metrics.txt",
    "Logistic Regression": "D4_logreg_metrics.txt",
    "KNN": "D4_knn_metrics.txt"
}
...
for model_name, file_path in files.items():
    ...
    if metrics:
        summary_data.append({
            ...
            "F1_raw": metrics['F1 Score'], # keep for sorting/CES
...
def compute_ces(score, co2):
    if co2 is None or co2 == 0 or np.isnan(co2):
        return 0.0
    # Formula: score * log1p(1 / (co2 + 1e-12))
...
# ── Save Files ───────────────────────────────────────────────────────────────
df_summary.drop(columns=["F1_raw"]).to_csv("model_comparison.csv", index=False)
...
bars = plt.bar(df_ces_sorted["Model"], df_ces_sorted["CES"], color=colors)

plt.xticks(rotation=45, ha='right')
plt.title("Convergence Efficiency Score (CES) - Higher is Better")
plt.ylabel("Efficiency Score (Metric * log1p(1/CO2))")
plt.xlabel("Model")
plt.grid(axis='y', linestyle='--', alpha=0.7)
plt.tight_layout()
plt.savefig('ces_comparison_bar_chart.png', dpi=300)
print(f"\nVisualization saved to 'ces_comparison_bar_chart.png'")

print("\nEntire aggregation process completed successfully.")
