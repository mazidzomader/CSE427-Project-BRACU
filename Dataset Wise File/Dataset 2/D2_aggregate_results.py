...
files = {
    "MLP": "D2_mlp_metrics.txt",
    "Random Forest": "D2_rf_metrics.txt",
    "XGBoost": "D2_xgb_metrics.txt",
    "AdaBoost": "D2_ada_metrics.txt",
    "Logistic Regression": "D2_logreg_metrics.txt",
    "KNN": "D2_knn_metrics.txt"
}
...
# ── Save Files ───────────────────────────────────────────────────────────────
df_summary.drop(columns=["F1_raw"]).to_csv("D2_model_comparison.csv", index=False)
df_ces_sorted.to_csv("D2_ces_scores.csv", index=False)

with open("D2_final_summary.txt", "w", encoding="utf-8") as f:
    f.write("="*80 + "\n")
    f.write("               MODEL COMPARISON TABLE (DATASET 2)\n")
    f.write("="*80 + "\n")
...
plt.tight_layout()
plt.savefig('D2_ces_comparison_bar_chart.png', dpi=300)

print("\nAggregation process completed. Results saved to D2_final_summary.txt and D2_ces_comparison_bar_chart.png")
