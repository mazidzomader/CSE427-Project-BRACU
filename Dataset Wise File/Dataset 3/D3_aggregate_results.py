...
# ── Configuration (Dataset 3) ───────────────────────────────────────────────
files = {
    "MLP": "D3_mlp_metrics.txt",
    "Random Forest": "D3_rf_metrics.txt",
    "XGBoost": "D3_xgb_metrics.txt",
    "AdaBoost": "D3_ada_metrics.txt",
    "Logistic Regression": "D3_logreg_metrics.txt",
    "KNN": "D3_knn_metrics.txt"
}
...
df_summary.drop(columns=["F1_raw"]).to_csv("D3_model_comparison.csv", index=False)
df_ces.to_csv("D3_ces_scores.csv", index=False)

with open("D3_final_summary.txt", "w", encoding="utf-8") as f:
...
plt.xticks(rotation=45, ha='right'); plt.title("D3 CES Comparison (Stroke Data)"); plt.ylabel("Score")
plt.tight_layout(); plt.savefig('D3_ces_comparison_bar_chart.png', dpi=300); plt.close()

print("\nAggregation completed. Results saved to D3_final_summary.txt")
